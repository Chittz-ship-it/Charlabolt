...
const PH = 18;
const GRAV = 0.48;
const INTERIOR_FLOOR_Y = GH - 20; // train interior floor

const UPGRADE_DATA = {
  jumpPower:  { name:"JUMP BOOST",  desc:"Leap higher between platforms", emoji:"⬆", costs:[60,150,350,800,2000], max:5 },
...
const WIN_TOP  = 12; // window top offset from building top
const WIN_H    = 55; // window height (generous for entry)
const WIN_FLOOR= WIN_TOP + WIN_H; // window sill = interior floor offset
...
function drawHoppy(ctx, px, py, frame, color, onGround, vy, hatId) {
  ctx.save();
...
function drawBuilding(ctx, b, frame) {
  ...
  // Entry window overlays (smashable left/right sides)
  const entryW=36;
  const drawEntryWindow=(ex, broken)=>{
    if(broken){
      // Broken frame only
      ...
      // Jagged shards remaining
      ctx.fillStyle="#5a3e1a"; ctx.fillRect(ex,top+WIN_TOP,2,WIN_H/3); ctx.fillRect(ex,top+WIN_TOP+WIN_H-WIN_H/4,2,WIN_H/4);
      ctx.fillRect(ex+entryW-2,top+WIN_TOP,2,WIN_H/2);
    } else {
  ...
  drawEntryWindow(b.x, b.leftWindowBroken);
  drawEntryWindow(b.x+b.w-entryW, b.rightWindowBroken);
}

function drawBuildingInterior(ctx, b, frame) {
  const top=GH-b.h;
  const ceilY=top+WIN_TOP, floorY=top+WIN_FLOOR;
  const intH=floorY-ceilY;
  // Room fill
  ...
  ctx.fillStyle="#5a3e1a55"; ctx.fillRect(b.x,ceilY+4,b.w,2);
  // Floor boards
  ctx.fillStyle="#3a2000cc"; ctx.fillRect(b.x,floorY-5,b.w,5);
  for(let i=0;i<Math.ceil(b.w/16);i++){
    ctx.fillStyle="#2a130088"; ctx.fillRect(b.x+i*16,floorY-5,1,5);
  }
  // Wallpaper strips
  ...
  for(let i=0;i<Math.floor(b.w/55);i++){
    ...
    ctx.fillStyle="#ffe09066"; ctx.beginPath(); ctx.arc(lx+1,ceilY+14,2,0,Math.PI*2); ctx.fill();
  }
  // Scrolling portraits / decorations on wall (parallax)
  for(let i=0;i<Math.floor(b.w/40);i++){
    const px2=b.x+20+i*40;
    ctx.fillStyle="#33200888"; ctx.fillRect(px2,ceilY+8,16,intH-18);
    ctx.strokeStyle="#5a3e1a44"; ctx.lineWidth=1; ctx.strokeRect(px2,ceilY+8,16,intH-18);
  }
}

function drawBuildingForeground(ctx, b) {
  const top=GH-b.h;
  // Thick side walls so Hoppy looks enclosed
  ctx.fillStyle="#2a1e10"; ctx.fillRect(b.x,top,10,b.h);
  ctx.fillRect(b.x+b.w-10,top,10,b.h);
...
export default function App() {
  ...
  const generatePlatforms=()=>{
    ...
    while(cx<GW+900){
      ...
      if(Math.random()>0.45){
        const h=140+Math.random()*80;
        platforms.push({type:"building",x:cx,w,h,leftWindowBroken:false,rightWindowBroken:false});
      } else {
        const h=72+Math.random()*18;
        platforms.push({type:"carriage",x:cx,w,h,glass:makeSkylights(cx,w),windowBarriers:[]});
      }
  ...
  const startGame=useCallback(()=>{
    ...
    gameRef.current={
      ...
      hasDbl:u.doubleJump>=1,dbLeft:0,onGround:true,
      layer:"roof",    // "roof" | "train" | "building"
      inTrain:null,inBuilding:null,
  ...
  const jump=useCallback(()=>{
    ...
    // No jumping inside buildings (corridor is too tight - it's a sprint-through)
    ...
    if(s.onGround||s.coyoteLeft>0){
      ...
      for(let i=0;i<6;i++) s.particles.push({x:PX+PW/2,y:s.py+PH,vx:(Math.random()-0.5)*4,vy:-Math.random()*2-0.5,life:18,maxLife:18,color:s.skinColor,size:2+Math.random()*2});
    } else if(s.dbLeft>0){
      s.vy=s.jumpPower*0.85; s.dbLeft=0;
      for(let i=0;i<12;i++) s.particles.push({x:PX+PW/2,y:s.py+PH/2,vx:(Math.random()-0.5)*8,vy:(Math.random()-0.5)*8,life:22,maxLife:22,color:s.skinColor,size:3+Math.random()*2});
    }
  },[]);
  ...
  useEffect(()=>{
    ...
    const tick=()=>{
      ...
      s.vy+=GRAV; s.py+=s.vy;
      s.trail.unshift({x:PX,y:s.py}); if(s.trail.length>12)s.trail.pop();
      ...
      if(last&&last.x+last.w<GW+900){
        const w=200+Math.random()*200,gap=60+Math.random()*60+s.speed*1.5,nx=last.x+last.w+gap;
        if(Math.random()>0.45){
          const h=140+Math.random()*80;
      ...
      if(s.layer==="roof"){
        ...
        for(const b of s.platforms){
          ...
          if(b.type==="carriage"){
            ...
            if(PX+PW>b.x+2&&PX<b.x+b.w-2&&s.py+PH>=bTop&&s.py+PH<=bTop+Math.abs(s.vy)+8&&s.vy>=0){
              ...
              else{s.py=bTop-PH;s.vy=0;landed=true;if(!s.onGround&&s.hasDbl)s.dbLeft=1;}
              break;
            }
          } else {
            // BUILDING
            // Roof landing (if Hoppy is above the building)
            if(PX+PW>b.x+2&&PX<b.x+b.w-2&&s.py+PH>=bTop&&s.py+PH<=bTop+Math.abs(s.vy)+8&&s.vy>=0){
            ...
            // Horizontal window entry — left side of building scrolls into Hoppy
            const winTopAbs=bTop+WIN_TOP, winBotAbs=bTop+WIN_FLOOR;
            if(!landed&&PX+PW>=b.x&&PX+PW<b.x+20&&s.py+PH>winTopAbs&&s.py<winBotAbs){
              // Enter building horizontally!
              b.leftWindowBroken=true;
              spawnGlass(s,b.x+10,winTopAbs+WIN_H/2);
              s.py=winBotAbs-PH; s.vy=0;
              s.layer="building"; s.inBuilding=b; s.onGround=true;
              break;
            }
          }
        }
        ...
        if(b.x+b.w<PX+PW){s.layer="roof";s.inTrain=null;s.py=floorY-PH;s.onGround=false;}
          ...
          if(s.vy>=0&&s.py+PH>=floorY){s.py=floorY-PH;s.vy=0;s.onGround=true;if(s.hasDbl)s.dbLeft=1;}
        ...
        const b=s.inBuilding;
        const bTop=GH-b.h, floorY=bTop+WIN_FLOOR;
        // Lock to corridor floor
        s.py=floorY-PH; s.vy=0; s.onGround=true;
        // Exit right side — smash through right window
        if(b.x+b.w-10<=PX+PW){
          b.rightWindowBroken=true;
          spawnGlass(s,b.x+b.w-5,bTop+WIN_TOP+WIN_H/2);
          s.layer="roof"; s.inBuilding=null; s.onGround=false;
          s.vy=-1; // slight upward pop on exit
          s.coyoteLeft=s.coyote;
        }
      }
      ...
      for(let i=0;i<65;i++){
        const sx=((i*137.5+s.frame*s.speed*0.07)%(GW+10)+GW+10)%(GW+10),sy=(i*46.1)%(GH*0.42);
      ...
      ctx.beginPath();ctx.arc(GW*0.78,32,20,0,Math.PI*2);ctx.fill();
      ctx.fillStyle="#080514";ctx.shadowBlur=0;ctx.beginPath();ctx.arc(GW*0.78+7,28,17,0,Math.PI*2);ctx.fill();ctx.restore();
      // BG city
      ctx.fillStyle="#0c0820";
      for(let i=0;i<12;i++){
        const bx=((i*55-s.frame*s.speed*0.04)%(GW+80)+GW+80)%(GW+80)-40,bh=25+((i*37)%55);
      ...
      for(let i=0;i<6;i++){
        ...
        ctx.strokeStyle="#1e1635";ctx.lineWidth=2.5;ctx.beginPath();ctx.moveTo(tx,GH-4);ctx.lineTo(tx,GH-85);ctx.stroke();
      ...
      ctx.globalAlpha=1;
      if(!s.gameOver)drawHoppy(ctx,PX,s.py,s.frame,s.skinColor,s.onGround,s.vy,s.hatId);
      ...
      if(s.layer==="building"&&s.inBuilding){
        ...
        ctx.textAlign="center";ctx.fillStyle="#c9a84c";ctx.fillText("▶ INSIDE",GW/2+60,17);
        ctx.textAlign="left";
      }
      ...
      if(s.layer!=="building"){
        ...
        if(s.layer==="train"){ctx.textAlign="center";ctx.fillStyle="#ff9f1c";ctx.fillText("▼ TRAIN",GW/2+60,17);}
        ctx.textAlign="left";
      }
  ...
  if(screen==="game") return (
    <div style={S.page}><div style={{width:"100%",maxWidth:"600px"}}>
      ...
      <div style={{padding:"8px 12px",background:"#07060d",display:"flex",justifyContent:"space-between",fontSize:"10px",color:"#444"}}>
        <span>TAP / SPACE TO HOP • AIM FOR WINDOWS TO PASS THROUGH</span><span>◆ {credits}</span>
      </div>
    </div></div>
  );
      ...
      <div style={{display:"flex",flexDirection:"column",gap:"10px",width:"100%"}}>
        {Object.entries(UPGRADE_DATA).map(([key,data])=>{
          ...
          return(<div key={key} style={{...S.card,border:`1px solid ${flash===key?c:c+"22"}`,background:flash===key?c+"14":"#090815",transition:"all 0.3s",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            ...
            <div>{maxed?<span style={{fontSize:"10px",color:"#ffd93d"}}>MAXED</span>:<button style={{...S.btn(can),opacity:can?1:0.35,padding:"8px 14px",fontSize:"11px"}} onClick={()=>buyUpgrade(key)}>◆{cost}</button>}</div>
          </div>);
        })}
      </div>
      <div style={{display:"flex",gap:"10px"}}>
  ...
  return (
    <div style={S.page}><div style={S.wrap}>
      ...
      <div style={{textAlign:"center"}}>
        <div style={{fontSize:"10px",color:c+"88",letterSpacing:"6px",marginBottom:"8px"}}>ENDLESS RUNNER</div>
